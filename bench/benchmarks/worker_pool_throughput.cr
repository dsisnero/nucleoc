...
module NucleocBench
  module WorkerPoolThroughput
    def self.run(config : Config)
      report_header("Worker pool throughput", config)
      haystacks = haystacks_with_needle(
        config.dataset_size,
      ...
      pools = config.core_counts.map { |workers| {workers, Nucleoc::CMLWorkerPool.new(workers)} }
      ...
      Benchmark.ips(calculation: config.calculation, warmup: config.warmup) do |x|
        ...
        pools.each do |workers, pool|
          x.report("worker_pool workers=#{workers}") do
